...
export default async function handler(req, res) {
  try {
    ...
    if (action === "game_entry") {
      ...
      if (!cost) return res.status(400).json({ error: "Unknown game." });
      amount = -cost;
      type = "game_entry";
    } else if (action === "game_reward") {
      const requested = Math.floor(Number(req.body?.coins || 0));
      const max = MAX_REWARD_BY_GAME[gameKey] || 0;
      if (requested < 1 || requested > max) {
        return res.status(400).json({ error: "Reward is outside the allowed range." });
      }
      amount = requested;
      type = "game_reward";
    } else {
    ...
    return res.status(200).json({ wallet: data.wallet, transaction: data.transaction });
  } catch (error) {
    if (/insufficient/i.test(error.message || "")) error.status = 402;
