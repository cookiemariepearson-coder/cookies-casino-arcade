...
export default async function handler(req, res) {
  try {
    ...
    if (!/^[a-z0-9_]{3,24}$/.test(username)) {
      return res.status(400).json({ error: "Player name must use 3–24 letters, numbers, or underscores." });
    }
    ...
    if (error) {
      if (error.code === "23505") return res.status(409).json({ error: "That player name is already taken." });
      throw error;
    }
